...
//**********************************************************************************************
//*** functions by Manoj added for BitTorrent module
...
TransportAddress* InetUnderlayConfigurator::createBTInitialNode(const char* type,bool initialize)
{
    ...
    else if (!strcmp(type,"inet.applications.BitTorrent.BTHostSeeder"))
        device  = "i=device/mainframe_l";

	//following two lines commented by Manoj
//    node->setGateSize("in", 1);
//    node->setGateSize("out", 1);
	//following line is added instead of above two
	node->setGateSize("pppg", 1);
    ...
    // create meta information
    InetInfo* info = new InetInfo(0, node->getId(), NULL);
    AccessNet* accessNet= check_and_cast<AccessNet*>
    (accessNode[intuniform(0, accessNode.size() - 1)]
                ->getSubmodule("accessNet"));
    info->setAccessNetModule(accessNet);
	...
	//module parameters. 
	node->getSubmodule("tcpApp",0)->par("address").setStringValue(addrString.c_str());
	node->getSubmodule("tcpApp",0)->par("port") = address->getPort();
    }
	//else block is added by Manoj.
	else
